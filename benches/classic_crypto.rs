...
/// This was previously generated by [`generate_prime_order_group_plus_generator`]
...
pub fn generate_prime_order_group_plus_generator(security_lvl: u32) -> (Modulus, Zq) {
    let two = Z::from(2);
...
mod rsa_textbook {
    ...
    /// These constants were previously generated by [`rsa_textbook::generate(1024)`]
    ...
    /// 6. Output `(N, pk, sk)`.
    pub fn generate(security_lvl: u32) -> (Modulus, Z, Z) {
    ...
    /// Returns a pre-computed RSA key pair that was computed with `generate(1024)`.
    pub fn static_generate() -> (Modulus, Z, Z) {
...
/// benchmark [`rsa generate`] for 1024 bit security
pub fn bench_rsa_generate(c: &mut Criterion) {
    c.bench_function("RSA generate", |b| b.iter(|| rsa_textbook::generate(1024)));
}
...
mod dh_ke {
    ...
    /// pre-computed with `generate_prime_order_group_plus_generator(1024)`.
    pub fn static_generate_pp() -> (Modulus, Zq) {
    ...
    ///
    /// `dh_ke::generate_key_pair(modulus, generator) -> (pk, sk)`,
    /// where `pk = g^sk mod modulus` and `sk` uniformly random
    pub fn generate_key_pair(modulus: &Modulus, generator: &Zq) -> (Zq, Z) {
    ...
    /// 2. run one cycle of `generate_key_pair` and `combine_to_shared_sk` on each end (2 times),
    ///    i.e. one key exchange at both ends and compare the computed shared secrets
    pub fn dh_run() {
        let (modulus, generator) = static_generate_pp();

        let (pk_0, sk_0) = generate_key_pair(&modulus, &generator);
        let (pk_1, sk_1) = generate_key_pair(&modulus, &generator);
...
mod el_gamal_enc {
    ...
    /// Returns a pre-computed set of public parameters that was
    /// computed with `generate_prime_order_group_plus_generator(1024)`.
    pub fn static_generate_pp() -> (Modulus, Zq) {
        let generator = Zq::from_str(GEN_PRIME_ORDER_GROUP).unwrap();
        let modulus = generator.get_mod();
        (modulus, generator)
    ...
    /// 1. get (p, g) from previously generated public parameters
    /// 2. run one cycle of `generate_key_pair`, `enc`, `dec`, and compare the `msg` to the result
    pub fn el_gamal_run() {
        let (modulus, generator) = static_generate_pp();

        let (pk, sk) = generate_key_pair(&modulus, &generator);
