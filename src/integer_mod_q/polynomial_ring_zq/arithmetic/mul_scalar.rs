...
impl Mul<&Z> for &PolynomialRingZq {
    ...
    fn mul(self, scalar: &Z) -> Self::Output {
        let out = self.poly * (scalar.modulo(elf.modulus.get_q()));
        out.reduce();
        out
    }
}
...
impl PolynomialRingZq {
    ...
    pub fn mul_scalar_zq_safe(&self, scalar: &Zq) -> Result<Self, MathError> {
        ...
        let mut out = PolynomialRingZq::from(&scalar.modulus);
        unsafe {
...
mod test_mul_z {
    ...
    fn borrowed_correctness() {
        let poly_1 =
            PolynomialRingZq::from_str(&format!("3  1 2 {} mod {}", i64::MAX, u64::MAX)).unwrap();
