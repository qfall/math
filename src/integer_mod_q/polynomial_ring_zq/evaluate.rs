...
//! Implementations to evaluate a [`PolynomialRingZq`].
...
impl<Integer: Into<Z>> Evaluate<Integer, Zq> for PolynomialRingZq {
    /// Evaluates a [`PolynomialRingZq`] on a given input that implements [`Into<Z>`].
    ///
    /// Parameters:
    ...
    /// use std::str::FromStr;
    ///
    /// let poly = PolynomialRingZq::from_str("5  0 1 2 -3 1 / 6  1 2 3 4 5 6 mod 17").unwrap();
    /// let value = Z::from(3);
    ///
    ...
    fn evaluate(&self, value: Integer) -> Zq {
        PolyOverZq::from((&self.poly, self.modulus.get_q())).evaluate(value)
    }
}

impl PolynomialRingZq {
    /// Evaluates a [`PolynomialRingZq`] on a given input of [`Zq`]. Note that the
    ...
    pub fn evaluate_safe(&self, value: &Zq) -> Result<Zq, MathError> {
        if self.modulus.get_q() != Z::from(&value.modulus) {
...
impl Evaluate<&Zq, Zq> for PolynomialRingZq {
    /// Evaluates a [`PolynomialRingZq`] on a given input of [`Zq`]. Note that the
    ...
    /// Use [`PolynomialRingZq::evaluate_safe`] if a panic has to be avoided.
    ///
    /// Parameters:
    /// - `value`: the value with which to evaluate the polynomial.
    ///
...
implement_for_owned!(Zq, Zq, PolynomialRingZq, Evaluate);
