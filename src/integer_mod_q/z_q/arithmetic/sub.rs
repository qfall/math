...
impl Zq {
    ...
    pub fn sub_safe(&self, other: &Self) -> Result<Zq, MathError> {
        ...
        unsafe { Ok(self.sub_unsafe(other)) }
    }

    /// This function behaves exactly as [`Zq::sub_safe`] with the exception that the modulus is not compared between the two types.
    ///
    /// # Safety
    /// As an developer make sure that this function is only used if the modulus has been
    /// tested before.
    pub(crate) unsafe fn sub_unsafe(&self, other: &Self) -> Self {
