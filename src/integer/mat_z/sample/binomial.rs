...
impl MatZ {
    ...
    ///
    /// let sample = MatZ::sample_binomial(2, 2, -1, 2, 0.5).unwrap();
    /// ```
    ///
    /// # Errors and Failures
...
mod test_sample_binomial {
    ...
    fn boundaries_kept_small() {
        for _ in 0..8 {
            let matrix = MatZ::sample_binomial(1, 1, -1, 2, 0.5).unwrap();
    ...
    #[test]
    fn false_size() {
        let _ = MatZ::sample_binomial(0, 3, 0, 1, 0.5);
    }
    ...
    #[test]
    fn availability() {
        let _ = MatZ::sample_binomial(1, 1, 0u8, 1u16, 7u8);
        let _ = MatZ::sample_binomial(1, 1, 0u16, 1u32, 7u16);
        let _ = MatZ::sample_binomial(1, 1, 0u32, 1u64, 7u32);
        let _ = MatZ::sample_binomial(1, 1, 0u64, 1i8, 7u64);
        let _ = MatZ::sample_binomial(1, 1, 0i8, 1i16, 7i8);
        let _ = MatZ::sample_binomial(1, 1, 0i16, 1i32, 7i16);
        let _ = MatZ::sample_binomial(1, 1, 0i64, 1i64, 7i32);
        let _ = MatZ::sample_binomial(1, 1, 0, Z::ONE, 7i64);
        let _ = MatZ::sample_binomial(1, 1, 0, 1u8, 0.5f32);
        let _ = MatZ::sample_binomial(1, 1, 0, 1u8, 0.5f64);
        let _ = MatZ::sample_binomial(1, 1, 0, 1, Q::from((1, 2)));
    }
    ...
    #[test]
    fn matrix_size() {
        let mat_0 = MatZ::sample_binomial(3, 3, 0, 1, 0.5).unwrap();
        let mat_1 = MatZ::sample_binomial(4, 1, 0, 1, 0.5).unwrap();
        let mat_2 = MatZ::sample_binomial(1, 5, 0, 1, 0.5).unwrap();
        let mat_3 = MatZ::sample_binomial(15, 20, 0, 1, 0.5).unwrap();
