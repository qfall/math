...
arithmetic_trait_borrowed_to_owned!(Add, add, MatQ);
arithmetic_trait_mixed_borrowed_owned!(Add, add, MatQ);
...
mod test_add {
    ...
    #[test]
    fn add_borrow() {
        let a: MatQ = MatQ::from_str(&String::from("[[1, 2, 3],[3, 4, 5]]")).unwrap();
        let b: MatQ = MatQ::from_str(&String::from("[[1, 2, 3],[3, -4, 5]]")).unwrap();
        let c: MatQ = &a + &b;
        assert!(c == MatQ::from_str(&String::from("[[2, 4, 6],[6, 0, 10]]")).unwrap());
    }
