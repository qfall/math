...
mod test_mul {
    ...
    fn different_dimensions_correctness() {
        let mat = MatQ::from_str("[[2,1],[1,2]]").unwrap();
        let vec = MatQ::from_str("[[1],[0]]").unwrap();
        let cmp = MatQ::from_str("[[2],[1]]").unwrap();
    ...
    fn incompatible_dimensions() {
        let mat_1 = MatQ::from_str("[[2,1],[1,2]]").unwrap();
        let mat_2 = MatQ::from_str("[[1,0],[0,1],[0,0]]").unwrap();
