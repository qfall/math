    ...
    pub fn cholesky_decomposition(&self) -> MatQ {
        ...
        for i in 0..self.get_num_rows() {
            for j in 0..(i + 1) {
                let mut s: f64 = 0.0;
                for k in 0..j {
                    s += f64::from(&out.get_entry(i, k).unwrap())
                        * f64::from(&out.get_entry(j, k).unwrap());
                }
                if i == j {
                    out.set_entry(i, j, (f64::from(&self.get_entry(i, i).unwrap()) - s).sqrt())
                        .unwrap();
                } else {
                    out.set_entry(
                        i,
                        j,
                        (f64::from(&self.get_entry(i, j).unwrap()) - s)
                            / out.get_entry(j, j).unwrap(),
                    )
                    .unwrap();
                }
            }
